...
class Strategy6(Testback):
    def __init__(self, data, order_bias, order_steps, stop_steps):
        super(Strategy6, self).__init__(data)
    ...
    def can_order(self):
        return self.num_order == 0 and self.row['nextbid1p_label_pred_i'] >= self.order_bias

    def order(self):
        self.order_price = self.lastest_bid1p
        self.num_order += 1
